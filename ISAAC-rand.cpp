...
void seed_random(char *term, int length) {
  ...
  strlcpy((char *)(R.randrsl), term, length);
  randinit(&R, TRUE);
}

short random_num(short max) { return rand(&R) % max; }

// overloads for parallel_omp.cpp

void seed_random(char *term, int length, randctx *ctx) {
  memset(ctx->randrsl, 0, sizeof(ctx->randrsl));
  strlcpy((char *)(ctx->randrsl), term, length);
  randinit(ctx, TRUE);
}

short random_num(short max, randctx *ctx) { return rand(ctx) % max; }
